...
class ManufacturerViewsTests(TestCase):
    ...
    def test_update_view_template(self):
        res = self.client.get(reverse("taxi:manufacturer-update", kwargs={"pk": 1}))
    ...
    def test_delete_view_template(self):
        res = self.client.get(reverse("taxi:manufacturer-delete", kwargs={"pk": 1}))
    ...
    def test_manufacturer_list_in_list_view(self):
        ...
        per_page = res.context["paginator"].per_page
        self.assertEqual(
            self.manufacturers[:per_page], list(res.context["manufacturer_list"])
        )
